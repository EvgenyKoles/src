...
func submitWords(n nextFunc, c chan string){
	for {
		word := n()
		
           	if word == "" {
			close(c) // как только кончаются слова на след горутину
			return
		}
		c <- word 
	}
...
func countWords(c chan string, l chan pair){
...
func fillStats(l chan pair)counter {
