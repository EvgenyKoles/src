...
------------------------------------------------------------------
	преобразование строки с (мусором) в число
...
------------------------------------------------------------------
	два числа с плавающей точкой переданы в строке.
	найти деление от них
...
func main() {
	...
	//d = strconv.ParseFloat(s[1], 64)

	t := []float64{}

	for i :=0; i < len(s); i++ {

		result, err := strconv.ParseFloat(s[i], 64)
		if err != nil {
		panic(err)
		} else {
			t = append(t, result)
...
func main() {
	...
	action(1,2, func(i1, i2 int) int {return i1+i2})



	d := func (r,t string) string  {return r+t}
...
-----------------------------------------------------------------------
проверка в большом числе на четность каждого сивола
...
fn := func(a uint) uint  {
    var s string = strconv.FormatUint(uint64(a), 10) // преобразовали число в строку
    var z string
    for _,v := range s {
        i,_ := strconv.Atoi(string(v)) // преобразовали в число
        if i != 0  && i%2 == 0 {
            q := strconv.Itoa(i)// обратно в строку
            z += q
...
// String makes Animal satisfy the Stringer interface. Тут написали как будет распечатываться этот тип структуры
