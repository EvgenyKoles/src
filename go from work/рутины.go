...
func main() {
	...
	//счетчик горутин станет нулевым. wg.Add(1)++, wg.Done()--. !!!!!!!!!!!!!!!!!!!
...
func main() {
	...
	// По идее значение счетчика должно быть 1000, 
	//но крайне вероятно, что этого не произойдет
	fmt.Println(x)
}
...
если одна горутина взяла Lock, 
то другие горутины этой операции выполнить уже не могут и вынуждены ожидать, 
пока взявшая Lock горутина не выполнит Unlock.
...
//А те горутины в которых выполняется fn() моментально передают 
//управление обратно и черт знает когда они там отработают
----------------------------	 
