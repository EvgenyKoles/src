...
// Важно, если нет доступных вариантов и нет блока default,
// подпрограмма заблокируется
...
func main() {
	...
	go func() {
		for {
			...
			case <-time.After(time.Second): // свич для каналов. ходит по каналам и спрашивает есть ли что то в этом канале, а в этом?
				fmt.Println("timeout")
			}
		}
	}()
