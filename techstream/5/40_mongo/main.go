...
type student struct {
	ID    bson.ObjectId `json:"id" bson:"_id"` //сериализуем
...
func main() {
	var err error
	sess, err = mgo.Dial("mongodb://localhost") // подключаемся
	PanicOnErr(err)

	// если коллекции(таблица) не будет, то она создасться автоматически
