...
func main() {
	...
	db, err = sql.Open("postgres", "user=postgres dbname=msu-go-11 sslmode=disable") // строка подключения
	PanicOnErr(err)

	// проверяем что подключение реально произошло ( делаем запрос ) пингуем что бы открыть соедниение
	err = db.Ping()
	PanicOnErr(err)

	// итерируемся по многим записям
	// Exec исполняет запрос и возвращает записи
	rows, err := db.Query("SELECT fio FROM students") // выбираем всех студентов
	PanicOnErr(err)
	for rows.Next() {
		var fio string
		err = rows.Scan(&fio) // сканируем
		PanicOnErr(err)
	...
	var lastID int64
	err = db.QueryRow( //вставляем запись
		"INSERT INTO students (fio, score) VALUES ($1, 0) RETURNING id",
		"Ivan Ivanov",
	).Scan(&lastID) // и сразу по нему скан
	PanicOnErr(err)
	...
	// символ ? является placeholder-ом. все последующие значения авто-экранируются и подставляются с правильным кавычками
	result, err := db.Exec(
		"UPDATE students SET info = $1 WHERE id = $2", // первое и второе значение 
		"test user",
		lastID,
	)
	PanicOnErr(err)
